...
##
library(AER)
modelelogit <- ivreg(ln_Share ~ Price + BindaGuerraQuality | Cost + BindaGuerraQuality, data = logitm)
summary(modelelogit)
...
mu <- -modelelogit$coefficients[[2]]
gamma <- modelelogit$coefficients[[3]]
inverse <- function(f,y) {
  fonction_racine <- function(x) {
    return(f(x)-y)
  }
...
for (i in 1:length(contrefactual$Binda_Price)) {
  contrefactual$Binda_cf_Volumes[i] <- Market_Size*gmu((alpha+contrefactual$Binda_Residuals[i]+gamma)/mu - 1 - contrefactual$Binda_Cost[i])/(1+gmu((alpha+contrefactual$Binda_Residuals[i]+gamma)/mu - 1 - contrefactual$Binda_Cost[i]) + gmu((alpha+contrefactual$Guerra_Residuals[i]+gamma)/mu - 1 - contrefactual$Guerra_Cost[i])+gmu((alpha+contrefactual$Speicher_Residuals[i])/mu - 1 - contrefactual$Speicher_Cost[i]))
}
contrefactual$Guerra_cf_Volumes <- rep(0, length(contrefactual$Binda_Cost))
for (i in 1:length(contrefactual$Binda_Price)) {
  contrefactual$Guerra_cf_Volumes[i] <- Market_Size*gmu((alpha+contrefactual$Guerra_Residuals[i]+gamma)/mu - 1 - contrefactual$Guerra_Cost[i])/(1+gmu((alpha+contrefactual$Binda_Residuals[i]+gamma)/mu - 1 - contrefactual$Binda_Cost[i]) + gmu((alpha+contrefactual$Guerra_Residuals[i]+gamma)/mu - 1 - contrefactual$Guerra_Cost[i])+gmu((alpha+contrefactual$Speicher_Residuals[i])/mu - 1 - contrefactual$Speicher_Cost[i]))
}
contrefactual$Speicher_cf_Volumes <- rep(0, length(contrefactual$Binda_Cost))
for (i in 1:length(contrefactual$Binda_Price)) {
  contrefactual$Speicher_cf_Volumes[i] <- Market_Size*gmu((alpha+contrefactual$Speicher_Residuals[i])/mu - 1 - contrefactual$Speicher_Cost[i])/(1+gmu((alpha+contrefactual$Binda_Residuals[i]+gamma)/mu - 1 - contrefactual$Binda_Cost[i]) + gmu((alpha+contrefactual$Guerra_Residuals[i]+gamma)/mu - 1 - contrefactual$Guerra_Cost[i])+gmu((alpha+contrefactual$Speicher_Residuals[i])/mu - 1 - contrefactual$Speicher_Cost[i]))
}
...
## Pour calculer l'harm il faut calculer les unités non-produites et non-vendues

Harm_Binda <- function(r) {
  sum((contrefactual$Binda_cf_Volumes - contrefactual$Binda_Volume)*(contrefactual$Binda_Price-contrefactual$Binda_cf_Price)*(1+r)^(as.numeric(Aujourdhui-contrefactual$Month)/365))/2
}
Harm_Binda(0.015)
Harm_Guerra <- function(r) {
  sum((contrefactual$Guerra_cf_Volumes - contrefactual$Guerra_Volume)*(contrefactual$Guerra_Price-contrefactual$Guerra_cf_Price)*(1+r)^(as.numeric(Aujourdhui-contrefactual$Month)/365))/2
}
Harm_Guerra(0.015)
...
Cf_Profits_Binda <- function(r) {
  sum((contrefactual$Binda_cf_Price - contrefactual$Binda_Cost)*contrefactual$Binda_cf_Volumes*(1+r)^(as.numeric(Aujourdhui-contrefactual$Month)/365))
}
Cf_Profits_Binda(0.015)

Cf_Profits_Guerra <- function(r) {
  sum((contrefactual$Guerra_cf_Price - contrefactual$Guerra_Cost)*contrefactual$Guerra_cf_Volumes*(1+r)^(as.numeric(Aujourdhui-contrefactual$Month)/365))
}
Cf_Profits_Guerras(0.015)

Harm_Speicher <- function(r) {
  sum((contrefsactual$Speicher_cf_Volumes - contrefactual$Speicher_Volume)*contrefactual$Speicher_cf_Price*(1+r)^(as.numeric(Aujourdhui-contrefactual$Month)/365))
}
Harm_Speicher(0.015)

firststeplogitmodel <- lm(Price ~ Cost + BindaGuerraQuality, data = logitm)